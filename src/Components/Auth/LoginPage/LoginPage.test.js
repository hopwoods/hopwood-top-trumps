...
import { createActor, fromPromise } from 'xstate'; // Added fromPromise
...
var mockAuthMachineSend = vi.fn();
var mockAuthMachineState; // Will be set in beforeEach for each test's specific scenario
vi.mock('../../../Hooks/UseAuthMachineState', function () { return ({
    useAuthMachineState: function () { return mockAuthMachineState; },
}); });
describe('LoginPage Component', function () {
    var appActor;
    var customAppMachine; // Define here to be accessible in renderLoginPage
    beforeEach(function () {
        ...
        var mockCheckAuthStatusActor = vi.fn().mockResolvedValue(null); // Simulates no user initially
        var mockLogoutActor = vi.fn().mockResolvedValue(undefined);
        // Provide mock actors to the appMachine for actors it directly invokes
        customAppMachine = appMachine.provide({
            actors: {
                checkAuthStatusActor: fromPromise(function () { return mockCheckAuthStatusActor(); }),
                logoutActor: fromPromise(function () { return mockLogoutActor(); }),
    ...
    var renderLoginPage = function () {
        ...
        return render(_jsx(GlobalStateContext.Provider, { logic: customAppMachine, children: _jsx(LoginPage, {}) }));
    };
    it('renders the login form with only Google login button', function () {
        renderLoginPage();
    ...
    //   })
    // })
    it('calls send with LOGIN_WITH_GOOGLE when Google login button is clicked', function () {
        renderLoginPage();
        var googleButton = screen.getByRole('button', { name: /Sign in with Google/i }); // Updated text
        fireEvent.click(googleButton);
        expect(mockAuthMachineSend).toHaveBeenCalledWith({ type: 'SUBMIT_LOGIN_WITH_GOOGLE' });
    });
    it('displays an error message if present in app state', function () {
    ...
    // })
    it('disables Google login button when loading for Google login', function () {
